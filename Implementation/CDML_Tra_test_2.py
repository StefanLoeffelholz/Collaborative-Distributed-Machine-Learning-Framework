...
def generate_dataloader_one_third_of_data():
    ...
    dataset = Base_Dataloader.CustomImageDataset(labels=shuffled_labels[2], data=shuffled_trainsets[2],transform=transform)
...
#model.load_state_dict(torch.load("./param_after_central_lr_01", weights_only=True))
model.load_state_dict(torch.load("./param", weights_only=True))
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

tra = Federated_Learning_Tra(2, trainloader=trainloader,communication_protocoll=Impl_Methods.Communication, device=DEVICE, ML_Model=model)
